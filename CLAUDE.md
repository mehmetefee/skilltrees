...
`fetch` decodes bodies and hides 1xx responses. `tests/api/pwa.test.js`
covers the manifest, the worker's script and rules, page and tree metadata
(with hostile text in every field), robots.txt, the sitemap, the well-known
URLs and the speculation rules; `tests/pwa-browser.test.js` runs the same
things in Chromium, offline included.

Every page registers the service worker, so browser suites run under it.
A suite that intercepts requests with `page.route()` won't see what the
worker fetches; launch its context with `serviceWorkers: 'block'`
(`tests/README.md`).
