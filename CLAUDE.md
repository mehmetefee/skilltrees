...
- `PUBLIC_ORIGIN` — the site's external origin, e.g.
  ...
  used for security.txt's `Canonical`. It is also what switches on passkeys
  (their RP ID is its host) and provider sign-in. Unset is fine: password
  accounts only.
...
route answers 404 without it. Section numbers below are Level 3's. The
...
- **Browser suites** (`tests/*.test.js`, Playwright) cover CRUD and cycle
  ...
  passkeys (Chromium's virtual authenticator over CDP) and the account page; each fails on unexpected console errors or CSP
